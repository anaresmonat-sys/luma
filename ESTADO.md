...
- ✅ CHECKPOINT 2026-09-20 — Retoque del recorrido de venta (pedido del usuario): el onboarding ahora es motivo → "¿Qué te ayudaría más?" → pantalla real (se quitaron "momento" y el reconocimiento para las 4 opciones; el paywall cuenta 2 respuestas). Coach: sin charla de ejemplo; saludo de LUMA personalizado por motivo (+ nombre del Mapa de Poder) en `lib/coach-saludo.ts`, 3 frases de arranque que solo rellenan la caja (no gastan la prueba gratis), botón "Ver compatibilidad de signos" quitado del coach (sigue en Tarot). Verificado en preview 375px. Sin revisor (retoque de pantalla secundaria). Pendiente: subir a GitHub/Vercel cuando el usuario diga sí. Próximo: seguir con los retoques de la página de ventas que indique el usuario.

- ✅ CHECKPOINT 2026-09-20 — Carrusel "La app por dentro" (pedido del usuario): los puntos funcionan todos (scroll calculado a mano + frame activo por cercanía al centro; antes solo iban el 1º y el último), punto activo = píldora dorada, flechas ‹ ›, tocar un teléfono lo centra, el activo resalta y los demás se atenúan. Ahora son 6 pantallas: se añade "Mapa de poder" (`public/carrusel/mapa-poder.png`) y se regenera `tarot.png` con cartas reales (antes salían cajitas en blanco). Capturas con datos de ejemplo vía `captura2.mjs` (scratchpad; en Git Bash exportar MSYS_NO_PATHCONV=1 o convierte /app/... en ruta de Windows). Verificado en preview. Pendiente: subir cuando el usuario diga sí; coach/descifrar/diario/inicio.png siguen con captura anterior.

- ✅ CHECKPOINT 2026-09-20 — Capturas del carrusel actualizadas con la app de hoy (descifrar, coach, diario con resultados REALES de la IA; tarot y mapa-poder con datos de ejemplo). Defecto real hallado y corregido al capturar: el Coach devolvía markdown (**HECHOS**) y respuestas cortadas; `app/api/coach/route.ts` ahora exige texto plano, ≤3 párrafos cortos, cierre con pregunta, max_tokens 600. Duda abierta del usuario: el chip "descifrar la conversación" de la sección "Cómo funciona" (components/landing/Solucion.tsx) parece botón pero es solo la etiqueta del mecanismo — pendiente de su decisión.

- ✅ CHECKPOINT 2026-09-20 — Cierre de la landing (`components/landing/CtaFinal.tsx`): fondo ciruela oscuro con filo y resplandor dorado en vez del bloque crema invertido del kit (elegido por el usuario, opción 2; rompía la atmósfera de LUMA). Verificado en preview 375px.

- ✅ CHECKPOINT 2026-09-20 — Decisiones del usuario: (1) El Círculo opción 2: sin plan, solo la PRIMERA persona agregada (la más antigua) se ve completa; las demás muestran solo el arquetipo emocional y el resto difuminado con texto de relleno (el real no llega al DOM) + botón "Desbloquea su lectura completa" → /paywall. `planActivo()` en `lib/prueba-gratis.ts` (simulación de `desbloquearPorPlan`; se reemplaza por el plan real de Hotmart). Limitación honesta: el texto real sigue en localStorage/Supabase; el blindaje real es del servidor cuando haya planes. (2) Prueba gratis: se MANTIENEN las dos (un uso gratis sin registro + 3 días de prueba del plan); sin cambios de código. Verificado en preview 375px; sin subir aún.

- ✅ CHECKPOINT 2026-09-20 — Retoques de la oferta (pedido del usuario): borde de la caja de garantía ahora dorado continuo (`Hairline completo` en `components/landing/ui.tsx`, solo en Garantia.tsx); `CtaButton` pasa de alto fijo a `min-h` + `py-3 text-center` para que el texto largo ("Descifrar mi primera conversación") se centre y crezca en 2 líneas en vez de desbordar la caja. El botón del plan Mensual se mantiene en contorno (señuelo de precio: el Anual es el dorado). Verificado en preview 375px; pendiente de subir tras ver el usuario.

- ✅ CHECKPOINT 2026-09-20 — Sección Agitación (`components/landing/Agitacion.tsx`, pedido del usuario: "iconos y frases descolgados"): las 3 frases pasan a línea de tiempo con el mismo `IconChip` de 44px que las cajas de §2 (iconos en la misma columna, verificado x=36 en las 7), hilo vertical entre chips, más aire tras la línea separadora y la última frase (el giro) resaltada con acento. Descartada la opción "respuesta bajo cada caja" (3 frases vs 4 cajas, no son respuestas 1 a 1). Verificado en preview 375px; pendiente de subir tras ver el usuario.

- ✅ CHECKPOINT 2026-09-20 — Corte entre el hero y "¿Te suena?" (idea del usuario): la caja de la demo (`components/landing/Hero.tsx`) quedaba pegada al borde de la sección con un borde al 18 %, y parecía cortada. Ahora borde dorado al 55 % en toda la caja + `pb-10 md:pb-16` de aire debajo. El degradado entre fondos (opciones 1/2 propuestas) queda sin aplicar; el usuario prefirió cerrar la caja. Verificado en preview (móvil y escritorio); pendiente de subir. Observación pendiente: en pantallas grandes el hero/demo se ve pegado a la izquierda (no centrado), sin tocar.

- ✅ CHECKPOINT 2026-09-20 — Retoques finos de la landing (pendientes de subir): (1) título de la oferta ahora "Menos que una consulta con tarotista o coach" (`app/page.tsx`); (2) la línea entre "¿Te suena?" y la línea de tiempo queda con el mismo aire arriba y abajo (~64 px en escritorio a cada lado: `pb-4 md:pb-6` en Problema.tsx + `pt-12 md:pt-16` en Agitacion.tsx). El centrado horizontal estaba bien (verificado con captura de 1505 px y con la captura del usuario). Capturas de escritorio: `desk.mjs` en el scratchpad (viewport 1204 css a 1.25x).

- ✅ CHECKPOINT 2026-09-20 — Misma corrección en la unión hero → "¿Te suena?": aire igual a cada lado de la línea (96 px escritorio / 64 px móvil) con `pb-16 md:pb-24` en la caja de la demo (`Hero.tsx`). Medido en preview (96/96). Pendiente de subir junto con el título "…tarotista o coach" y el ajuste de Problema/Agitación.

- ✅ CHECKPOINT 2026-09-20 — Línea entre "¿Te suena?" y la línea de tiempo eliminada (pedido del usuario): `SectionShell` gana `sinFiloSuperior` (Agitacion lo usa, ya que sigue a otra sección del mismo color) y Agitacion se solapa 1px (`-mt-px`) para tapar la costura de sub-píxel que aparece con escalado 125 %; el aire entre ambas queda en un solo espacio (48 px móvil / 64 px escritorio). Verificado con captura a 1505 px. Pendiente de subir.

- ✅ CHECKPOINT 2026-09-20 — Unión hero → "¿Te suena?" sin línea (idea del usuario: degradar el segundo bloque): `SectionShell` gana `degradadoSuperior` (fondo que arranca en `--bg` y se aclara a `--surface` en 160 px); Problema lo usa junto a `sinFiloSuperior`; el aire bajo la caja del hero baja a `pb-4 md:pb-8` para no duplicar espacio. Verificado a 1505 px y en móvil; pendiente de subir junto con la eliminación de la línea entre Problema y Agitación.

- ✅ CHECKPOINT 2026-09-20 — Degradado muy suave en TODAS las uniones de la landing (pedido del usuario): `SectionShell.degradadoSuperior` (200 px, de --bg a --surface o al revés según la sección) aplicado a Solución, La app por dentro, Oferta, Garantía y Preguntas (+ Problema ya hecho); sin filos claros de arriba. Cierre final (`CtaFinal.tsx`): gradiente bg→surface→bg sin el filo dorado ni la franja rojiza, y el resplandor dorado encogido para que no corte arriba. Verificado con capturas de página completa a 1204 px y 375 px. Pendiente de subir.

- ⚠️ DECISIÓN 2026-09-20 — Enlace publicado NO abierto al público a propósito: Vercel Authentication (Deployment Protection) sigue ACTIVA en el proyecto `luma`, así que quien no tenga sesión de Vercel ve la pantalla de acceso a Vercel (comprobado con curl: 302 a vercel.com/sso-api). La usuaria lo dejará así hasta terminar la app y usa la vista previa local para el móvil. Antes de abrirla: (1) apagarla en Vercel → luma → Settings → Deployment Protection → Vercel Authentication; (2) poner límite de uso en el servidor a las rutas `/api/*` de IA (hoy solo hay control en el navegador + tope de $10 en Anthropic). Compartir siempre con `https://` delante.

- ✅ 2026-09-20 — Título del carrusel: "Entra y mira cómo se siente" → "Desliza y mira cómo se siente" (`app/page.tsx`; el usuario ya está dentro de la página, "entra" no tenía sentido). Verificado en preview; pendiente de subir.

- ✅ 2026-09-20 — Defecto real hallado desde el carrusel ("no se ve toda la pantalla en algunas cajitas"): el shell de las pantallas con barra inferior (`app/app/(tabs)/layout.tsx`) usaba `min-h-dvh`, así que una respuesta larga del coach empujaba la caja de escribir y la barra fuera de la pantalla. Ahora `h-dvh` (el contenido hace scroll dentro de `main`, la barra siempre abajo) y el chat del coach (`coach/page.tsx`) ya no usa `justify-end` (no se podía subir al desbordar) sino un separador `flex-1`, con auto-scroll al último mensaje. Verificado con respuesta larga real y en Inicio, Tarot, Diario y Más. Capturas del carrusel regeneradas (coach, tarot, diario, descifrar y mapa-poder; descifrar y mapa a 408×883 reducidas a 375×812 para que quepan). Carrusel: verificado que se desliza con toque (Input.dispatchTouchEvent). Pendiente de subir con el título "Desliza y mira cómo se siente".

- ✅ 2026-09-20 — Carrusel: las 6 capturas regeneradas a 430×932 (iPhone Pro Max, misma proporción que el marco 9:19,5) y convertidas a `.webp` (46–111 KB c/u; `app/page.tsx` apunta a `/carrusel/*.webp`, los `.png` se borraron): cada pantalla se ve ENTERA dentro de su teléfono (Descifrar con sus 2 botones, Coach con caja y barra, Tarot con 5 cartas reales + barra, Mapa de Poder completo, Diario con barra). En Tarot se muestran las 5 cartas tiradas (en la app real, la que aún no se tiró conserva el reverso de LUMA). Contador del Diario con `lining-nums` (el "1" en Cormorant parecía una "I"). Nota de captura: usar un perfil de Chrome nuevo por captura (el uso gratis se guarda en localStorage y redirige a /paywall).

- ✅ 2026-09-20 — Inicio y marco de la app en pantalla ancha (pedido del usuario; pendiente de subir): (1) nuevo `app/app/layout.tsx`: TODA la app interna queda en una columna centrada de 480 px máx. (con filo lateral fino ≥md) en vez de estirarse por el monitor; (2) Inicio: más aire (`gap-8`, "Abrir mi lectura de hoy" con `mt-6`), texto bajo la carta más ancho (`max-w-[16rem]`, 13 px) en `HeroDemoLuma.tsx`; (3) ventanas bajas: la carta y los espacios se reducen por altura (`@media (min-height:681px) and (max-height:760px)` y `max-height:680px`; rangos separados porque Tailwind ordena los `@media` arbitrarios y se pisaban); (4) resplandor de la carta con `closest-side` (tenía bordes rectos); (5) scroll interno del `main` fino y dorado (`scrollbar-width: thin`). Comprobado: ventana del usuario = 1224×803 físicos al 125 % ≈ 979×642 css → todo cabe (cabecera, ánimo, carta, botones, barra); móvil 430×932 sin cambios de layout. Causa de fondo: mi cambio a `h-dvh` del shell hacía que, con poca altura, `justify-between` colapsara los espacios.

- ✅ 2026-09-20 — Coach en pantalla ancha (pedido del usuario; pendiente de subir): el saludo y las 3 frases de arranque quedan ARRIBA (se quitó el separador `flex-1` que las empujaba al fondo; la conversación crece hacia abajo y el auto-scroll sigue al último mensaje); el nombre del saludo se capitaliza (`aperturaCoach`: "maggie" → "Maggie"); resplandores del fondo con `ellipse 60% …dvh` para que no se vean bordes rectos. Verificado a 1533×724 css (ventana del usuario) y 430×932 con respuesta real (que ya usa el Mapa de Poder: Tauro/Hierofante).

- ✅ 2026-09-21 — Acceso de la dueña para revisar la app (pedido del usuario): en `/entrar` aparece "🛠️ Entrar a revisar la app (solo en tu computadora)" → `/app`, únicamente cuando `NODE_ENV === 'development'` (no existe en la versión publicada). Recorrido comprobado en preview: landing → Entrar → botón → Inicio. Para entrar a la app PUBLICADA sigue haciendo falta el enlace del correo (Supabase Auth → URL Configuration con el dominio de Vercel, pendiente) y apagar Vercel Authentication (ver decisión del 2026-09-20).

- ✅ 2026-09-21 — Fondo cortado en Tarot (pedido del usuario; pendiente de subir): con el shell `h-dvh`, la raíz de Tarot/Diario/Más medía solo el alto visible (`min-h-0`), así que su resplandor (`absolute inset-0`) no cubría lo que hacía scroll. Ahora `min-h-min` en Tarot, Diario y Más (y sin `overflow-y-auto` propio en Diario): el contenedor crece con el contenido y el fondo llega hasta el final (verificado a 1533×724 con scroll al fondo). NO aplicar `min-h-min` al Inicio: su contenedor crece de más y los botones se salen (probado y revertido); en su lugar, en ventanas ≤680 px de alto el espacio reservado al aviso "Guardado" baja a `h-3` y ahora caben carta con nombre de 2 líneas + botones. Incluye el acceso "🛠️ Entrar a revisar la app" solo en desarrollo.

- ✅ 2026-09-21 — Miniaturas del Tarot nítidas (pedido del usuario: "se ve difuminado"): las cartas de 360 px se reducían a 44 px en el navegador y el grano de las láminas de 1909 salía como ruido. Ahora hay `public/tarot/mini/*.webp` (132 px, Lanczos, ~12 KB c/u, 78 archivos) y la lista de Tarot usa `tirada.imagen.replace('/tarot/','/tarot/mini/')`. El logo del reverso en miniatura pasa de `h-6` a `h-9` con brillo (era una línea fina que parecía "de fondo"). Verificado a 1533×724. Pendiente de subir. Duda abierta: el usuario dijo que "todas las páginas se ven difuminadas como de fondo" — puede referirse a estas miniaturas o a los resplandores ciruela del fondo; preguntado.

- ✅ 2026-09-21 — Resplandor ciruela del fondo más tenue en toda la app (opción 2 del usuario; pendiente de subir): 19 valores bajados a ~55 % de su intensidad (Coach, Diario, Inicio, Tarot, Círculo, Compatibilidad, Descifrar, Mapa de Poder y los dos del `body` en `globals.css`: 55 % → 30 %). No se tocó el reverso de las cartas (75 %). Comparativa antes/después a 1533×724 en scratchpad `nuevas/antes-despues.png`. Si sigue pareciendo neblinoso: bajar aún más (o quitar el degradado fijo del `body`).

- ✅ 2026-09-21 — Causa real del "difuminado como de fondo": los resplandores de Diario, Tarot, Descifrar, Mapa de Poder, Círculo y Compatibilidad tenían ancho fijo en px (500 px) mayor que la columna, así que dibujaban un panel rectangular más claro con bordes rectos detrás del contenido (ya arreglado antes en Inicio y Coach con `closest-side` / `ellipse 60%`). Ahora `radial-gradient(ellipse 60% Ndvh at …)` en los 14 casos (se desvanece antes del borde). Se conserva la bajada de intensidad de antes (~55 %); si queda plano, subirla ~10 puntos. Verificado en la vista previa (Diario, Tarot, Descifrar). Pendiente de subir.

- ✅ 2026-09-21 — Sinergia zodiacal: selector nuevo (idea del usuario: opción 1 + 3; pendiente de subir). `app/app/compatibilidad/page.tsx`: pareja de círculos arriba ("Tú" en oro `--accent`, "Su signo" en rosa `--accent-2`, unidos por ♥), UNA lista de 12 signos debajo que toma el color del círculo activo (panel tintado + signo elegido resaltado), al elegir el propio pasa solo al de la otra persona, y si existe el Mapa de Poder el círculo "Tú" viene con su signo ya puesto (`leerMapaPoder`, en `useEffect`) con la nota "toca el círculo para cambiarlo". Verificado en preview 375 px: vacío, elegir Tauro→Escorpio, y con Mapa de Poder.

- ✅ 2026-09-21 — Sinergia zodiacal, ajuste pedido: panel y cajas de los 12 signos con borde dorado (`--accent` 70 % / 45 %), "ELIGE SU SIGNO" en dorado; solo el signo elegido toma el color del círculo (oro para "Tú", rosa `--accent-2` para "Su signo"). Verificado en preview 375 px. Pendiente de subir.

- ✅ 2026-09-21 — Más y "sin aire abajo" (pedido del usuario; pendiente de subir): (1) Más: se quitó la tarjeta "Tu arcano" (el Mapa de Poder sigue entrando por Tarot → "Conócete a ti misma"; se retiró el estado `mapaPoder`); notificaciones/ayuda/términos/privacidad van abajo (`mt-auto`), el hueco queda en medio — pendiente proponer algo útil ahí (p. ej. tarjeta de plan más completa). (2) Revisión a 430×932 de todas las pantallas: Inicio/Coach/Tarot ya llenaban; se arreglaron Diario y Descifrar (la caja de escribir se estira con `flex-1`, sin `justify-center`; mientras hay resultado deja de estirarse), Sinergia (círculos/casillas más grandes en ≥860 px de alto y "Calcular" abajo del todo hasta que hay resultado) y Círculo (estado vacío centrado con `my-auto`). Verificado con resultados reales de IA en Descifrar y Diario. Sigue pendiente subir todo lo del 2026-09-21.

- ✅ 2026-09-21 — Reordenado (pedido del usuario, opción 1; pendiente de subir): Tarot ahora solo empieza con la pregunta y las 5 tiradas; al final van "Repetir tu última tirada" y, bajo el título "MÁS LECTURAS", Sinergia zodiacal. "Conócete a ti misma" (Mapa de Poder) pasa al Diario como tarjeta bajo "Ver mi patrón" (`diario/page.tsx`); el botón atrás del Mapa de Poder vuelve al Diario (`volverHref="/app/diario"`). El Mapa de Poder ya solo se entra desde el Diario (y el Círculo cuelga de él); Sinergia solo desde Tarot y desde la sugerencia del Coach. Verificado en preview.

- ✅ 2026-09-21 — Segunda puerta al Mapa de Poder (opción 2 del usuario; pendiente de subir): Tarot → "MÁS LECTURAS" ahora tiene Sinergia zodiacal + "Conócete a ti misma" (`/app/mapa-poder?desde=tarot`). El botón atrás del Mapa de Poder vuelve a donde se entró: Tarot si viene con `?desde=tarot`, Diario por defecto. La barra inferior se queda en 5 destinos (no se añade un sexto icono: se apretaría en móvil y el Mapa se calcula una sola vez, no es de uso diario).

- ✅ CHECKPOINT 2026-09-22 — Fase actual: retoques de la app interna sobre lo publicado ayer (2026-09-21, commit 8ee0c39). Todo lo de la sesión del 21 sigue SIN subir (solo en local, verificado en preview): fondo ciruela sin panel rectangular en Diario/Tarot/Descifrar/Mapa de Poder/Círculo/Compatibilidad (`ellipse 60%`) + intensidad general bajada (~55%); miniaturas nítidas de Tarot (`public/tarot/mini/*.webp`, 132px) y logo del reverso más grande; Más sin la tarjeta "Tu arcano" (queda hueco en el medio, pendiente de decidir qué poner ahí — propuesta: tarjeta de plan más completa); Sinergia zodiacal rediseñada (pareja de círculos Tú/oro · Su signo/rosa, lista única con borde dorado, autocompleta con el Mapa de Poder); Diario/Descifrar/Círculo sin aire muerto abajo; reordenado Tarot (solo pregunta+5 tiradas arriba; "Repetir tirada" + "MÁS LECTURAS" con Sinergia y Conócete a ti misma al final) y Diario (tarjeta "Conócete a ti misma" añadida bajo "Ver mi patrón"); Mapa de Poder con 2 puertas (Diario y Tarot) y el botón atrás vuelve a la de origen; acceso "🛠️ Entrar a revisar la app" en /entrar (solo NODE_ENV=development). Próximo paso: el usuario está revisando todo en preview; a la espera de su "sí, sube" o de más ajustes.

- ✅ 2026-09-22 — Lecturas de tarot por categoría, no siempre sentimentales (pedido del usuario, defecto real confirmado): `app/api/tarot/route.ts` ya no dice "coach experta en relaciones sentimentales" fijo; ahora es "tarotista profesional" y recibe una `categoria` (amor/ruptura/decision/autoconocimiento/carta-del-dia) con su propio enfoque de lectura (`ENFOQUE_POR_CATEGORIA`), inspirado en cómo separan categorías las apps de tarot con más ventas (Tarotal, Tarot Master). `tarot/page.tsx` manda `categoria: id` al pedir la lectura. Verificado con IA real: Autoconocimiento leyó intuición/voz interna sin mencionar pareja; Decisión leyó sobre dar un paso en la vida sin asumir romance. Pendiente de subir junto con todo lo demás del 21-22.

- ✅ 2026-09-22 — Coach ya no fuerza el enfoque sentimental (mismo defecto que el tarot, corregido igual): `app/api/coach/route.ts` — LUMA pasa de "coach experta en relaciones sentimentales" a "experta en inteligencia emocional y en cómo nos relacionamos (pareja, familia, amistad, una misma)"; la estructura HECHOS/RIESGO/ACCIÓN ya no es obligatoria para todo mensaje, solo cuando la usuaria pega o describe una conversación real con alguien. Verificado con IA real: mensaje sobre un conflicto con la jefa respondió enteramente sobre eso (límites laborales, cómo hablarlo), sin desviarse a pareja. Pendiente de subir junto con todo lo del 21-22.

- ✅ 2026-09-22 — Iconos de las tiradas de Tarot (pedido del usuario): 🔀 y 🪞 traían su propio fondo de color pegado al glifo, desentonando con el resto de iconos de la app (formas sueltas sin fondo: ❤️💔☀️, y también 🏠💬🔮📔 de la barra). Reemplazados por 🧭 (Decisión) y 🪷 (Autoconocimiento), ambos sin fondo — `lib/seed-datos.ts`. Verificado en preview.

- ✅ 2026-09-22 — "LUMA" en la cabecera del Coach pasa de blanco (`--text-primary`) a dorado (`--accent-lite`), como en el resto de la app. `app/app/(tabs)/coach/page.tsx`. Verificado en preview.

- ✅ 2026-09-22 — Tarot: quitado el bloque "¿Primera vez aquí? Empieza con tu carta del día…" (repetía la fila "Carta del día" que ya es la última de la lista, con carta+cita duplicadas). `app/app/(tabs)/tarot/page.tsx`. Verificado en preview: la lista pasa directo a "MÁS LECTURAS" sin aire ni repetición. Pendiente de subir.

- ✅ 2026-09-22 — Tarot: tiradas de 3 cartas para Amor, Ruptura, Decisión y Autoconocimiento (pedido del usuario, "más pro", tras investigar apps top como Raka/Nummi/Jenova). `lib/tarotDeck.ts` → `POSICIONES_TIRADA` (3 posiciones por categoría, p.ej. Amor: "Lo que sientes tú / Lo que siente la otra persona / Hacia dónde va esto"). `app/api/tarot/route.ts` reescrito: recibe un array `cartas` (1 o 3) y lee VARIAS cartas como una sola historia conectada (palos repetidos, cartas que se suavizan/intensifican, posición), no como párrafos sueltos; sigue respetando `ENFOQUE_POR_CATEGORIA` del 2026-09-22 anterior. `TiradaGuardada`/`CartaTirada` en `tarot/page.tsx` ahora guardan un array de cartas + el texto combinado; nuevo componente `MiniCartaTirada` (tamaño fijo en px, no escala) para la fila de 3. Carta del día se queda en 1 sola carta (estándar del sector), sin cambios de UI. `leerGuardadas()` descarta datos del formato viejo para no romper con lecturas guardadas antes de este cambio. Verificado con IA real: Amor sacó 3 cartas (El Mago invertida, 8 de Copas, Reina de Bastos invertida) con lectura conectada de verdad; Carta del día sigue en 1 carta; Repetir tirada reabre sin gastar otro uso ni llamar a la IA de nuevo. Pendiente de subir.
