...
## RONDA 6 (RE-REVISIÓN) — 2026-09-13
Screenshot: docs/revisiones/tarot-375.png (localStorage limpio, sin `luma_ultima_tirada` — simula usuaria nueva sin historial; muestra el teaser de carta del día en la franja baja)
Usabilidad: 32/40
Craft: 16/20
Copy (si vende): N-A
Fidelidad (si hubo referencia): N-A
...
Detalle usabilidad: h1:3 h2:3 h3:3 h4:3 h5:3 h6:4 h7:3 h8:3 h9:3 h10:4
Detalle craft: jerarquía:3 profundidad:4 identidad:3 movimiento:3 encaje:3

Verificación del fix dirigido de esta ronda (defecto #1 de rondas 2-5 — franja baja sin contenido para el usuario sin historial):
- CORREGIDO Y VERIFICADO EN CÓDIGO Y EN SCREENSHOT. `page.tsx` líneas 152-173: bloque condicional `{!abierta && !ultima && (...)}` que renderiza exactamente en el escenario que las 4 rondas anteriores señalaron como techo (usuaria nueva, sin tirada abierta y sin historial guardado). Contenido: la MISMA `CartaSacerdotisa` (mismos datos de `LECTURAS_TAROT['carta-del-dia']` que ya usa la fila "Carta del día" de la lista — no es un dato inventado ni una ilustración nueva) a `scale-[0.6]` con `animar={false}` (correctamente estático, para no competir con la animación de "revelado" real que ocurre al abrir la tirada de verdad), + un texto de 2 líneas que explica POR QUÉ empezar por ahí ("es la tirada más corta y no necesita contexto previo" — razón concreta, no genérica), + un botón que llama a `alternar('carta-del-dia')`, la MISMA función que abren las filas normales — hereda el auto-scroll (`scrollIntoView` sobre `panelRef`) y el guardado en `localStorage`/`setUltima` ya verificados en rondas anteriores, sin código nuevo paralelo que pueda desincronizarse.
- Contenedor: `rounded-[var(--radius-card)]` (14-16px, la familia correcta de la ficha — a diferencia de la miniatura de 12px que sigue sin corregirse) + `border` con acento al 28% + `bg-[var(--surface)]`, el mismo token de superficie que usa el panel expandido y `TarjetaEjemplo` — profundidad consistente con el resto de la app, no un tratamiento ad-hoc.
- Confirmado visualmente en el screenshot: la franja que en las rondas 2-5 se veía plana y apagada (≈y 1050-1490 de 1567) ahora contiene la carta con su resplandor ámbar, el texto y el CTA dorado — la pantalla cerrada ya no tiene una zona "muerta" antes del nav. Este es el defecto estructural que 4 rondas consecutivas (2, 3, 4, 5) señalaron sin ambigüedad como EL punto a resolver — queda resuelto.
- BUG NUEVO (real, menor): el botón `Ver mi carta del día →` (línea 165-171) es un `<button>` con `className="text-[12.5px] font-bold text-[var(--accent-lite)]"` — SIN padding, SIN `min-h`, SIN `whileTap`/`motion.button`. A diferencia de: (a) las filas de tirada, que son `motion.button` con `whileTap={{scale:0.98}}`; (b) el chip "Repetir tu última tirada", que aunque también es un `<button>` plano tiene `px-4 py-3` (→ ≥44px de alto real); este nuevo botón no tiene ninguna de las dos protecciones — el área táctil real es solo la caja del texto (~18px de alto), por debajo de los "≥44px" de la Regla UX #5, y no hay ningún feedback visual al tocar (ni highlight, ni scale, ni color). Es una tercera variante de "botón de acción" en el mismo archivo, inconsistente con las otras dos. Fix: envolver en `AppLinkButton`/botón con `min-h-[44px]` + `px-4` + `whileTap={{scale:0.97}}`, o al menos igualar el patrón del chip vecino (padding suficiente para superar 44px).
- Matiz de copy: el texto "¿Primera vez aquí?" asume que el usuario nunca usó la pantalla; en rigor la condición es "`!abierta && !ultima`" (sin tirada abierta y sin historial EN ESTE NAVEGADOR), que también puede darse si el usuario limpió datos, cambió de dispositivo, o simplemente nunca tocó ninguna tirada aún en una sesión antigua — no es estrictamente "primera vez", aunque el caso de uso real dominante sí lo es. No se penaliza como error, se anota como matiz de precisión.

Impacto en las puntuaciones:
- Craft — profundidad: 3→4. Las rondas 2-5 atribuyeron EXPLÍCITAMENTE el techo de este eje a la franja plana sin superficie elevada (ronda 2 defecto #5: "Heurística 8 / Eje profundidad... consecuencia directa del defecto #1"). Con esa franja ahora ocupada por una superficie elevada real (borde + `var(--surface)` + resplandor), consistente con los otros 2 niveles de profundidad ya presentes en el resto de la pantalla (base con bloom + panel expandido), el eje deja de tener un defecto estructural identificable — se mueve a 4.
- h10 (ayuda contextual): 3→4. El bloque es, en sentido estricto, el "empty state que enseña" que faltaba: aparece exactamente en el caso sin historial, reutiliza un dato real (no inventa contenido), explica el POR QUÉ de la recomendación en una frase concreta, y ofrece un CTA directo a la acción. Cumple el criterio de la rúbrica ("empty states que enseñan... 0 pantalla muda") de forma más completa que antes, cuando el usuario sin historial no tenía ninguna orientación sobre por dónde empezar.
- h8 (estético/minimalista) se mantiene en 3, NO sube a 4: aunque el defecto de "pantalla vacía de valor" que lo capaba en rondas 2-5 está resuelto, el nuevo botón sin tratamiento de botón (raw, sin padding) es en sí mismo un elemento que "no se gana su lugar" con el mismo cuidado que el resto — no alcanza el nivel ejemplar en esta ronda.
- h1 y h4 se mantienen en 3: el bug nuevo del botón (sin feedback de tap, tercera variante de estilo de acción) es real pero acotado a un solo elemento, consistente con el nivel "bien, un ojo entrenado detecta qué afinar" que ya tenían estas heurísticas; no las hace caer por debajo de 3 porque el resto de la pantalla (filas, chip, panel) sigue teniendo feedback correcto.
- Resto de heurísticas y ejes de craft: sin cambios de código que los afecten; se mantienen en los valores de la ronda 5.

Diagnóstico: el fix dirigido que se pidió al cierre de la ronda 5 está genuinamente resuelto — el defecto estructural señalado sin ambigüedad en 4 rondas consecutivas (2, 3, 4, 5) ya no existe en el screenshot del caso más común (usuaria sin historial). Craft cruza su gate propio (16/20). Usabilidad sube a 32/40, +1 punto sobre la ronda 5, pero se queda 4 puntos por debajo del gate de 36. A diferencia de las rondas 2-5, donde había SIEMPRE un defecto concreto y dominante señalable con nombre y ubicación exacta, lo que queda ahora es un conjunto DISPERSO: 7 heurísticas distintas ancladas en 3/4 (h1, h2, h3, h4, h5, h7, h9), ninguna con un defecto único que la explique por sí sola — h7 está topada por decisión de alcance (un solo atajo, no varios; ampliar esto es agregar función nueva, no corregir un defecto), la lista de 5 opciones (vs. ≤4 recomendado) es una decisión de producto ya aceptada en 4 rondas sin que el equipo la reduzca, el radio de 12px de la miniatura es un desvío de 2-4px que ningún usuario promedio nota, y el resto son matices de comunicación/copy ya pulidos al límite razonable. El único defecto puntual y accionable de esta ronda (el botón nuevo sin padding/feedback) es fácil de arreglar, pero incluso corrigiéndolo el techo realista es de ~33-34/40, no 36 — porque el resto de la brecha no es UN bug sino la suma de siete heurísticas que ya están en su nivel "bien" y subir cada una a "ejemplar" exigiría cambios de alcance distintos e independientes entre sí (más atajos, copy aún más ajustado, reducir opciones de decisión, etc.), no una intervención puntual.

TOP DEFECTOS (ronda 6):
1. [Bloque teaser "Carta del día", botón "Ver mi carta del día →", línea 165-171] Botón de texto plano sin padding/min-height ni `whileTap` — tap target por debajo de 44px y sin ningún feedback visual al tocar, inconsistente con las 2 otras variantes de botón ya presentes en el mismo archivo (fila con `whileTap`, chip con padding ≥44px). Fix: usar el mismo patrón del chip vecino (padding suficiente) o `whileTap={{scale:0.97}}` como mínimo.
2. [Lista de 5 tiradas, decisión — heredado de rondas 3-5] Sigue un punto por encima de la guía de ≤4 opciones; no se tocó en ninguna de las 6 rondas. Fix: agrupar Amor+Ruptura bajo una sub-etiqueta, o mantenerlo si el dato de uso real lo sostiene.
3. [Miniatura de carta, radio 12px — heredado de rondas 3-5] Sigue siendo el único radio fuera de familia (12 vs 14-16 del resto, incluido el nuevo bloque teaser que sí usa `var(--radius-card)` correctamente). Ajuste fino no bloqueante.
4. [Heurística 7 — código, pantalla completa, heredado] El atajo sigue cubriendo un solo caso (última tirada); no hay un segundo default. Techo de alcance, no de calidad de ejecución.
5. [Copy "¿Primera vez aquí?" — bloque teaser, matiz] Se muestra también si el usuario simplemente limpió datos o aún no abrió ninguna tirada en este navegador, no estrictamente solo en una "primera vez" real. Matiz menor, no error.
...
¿Corresponde cerrar esta pantalla con criterio propio en esta ronda?
Sí. La ronda dirigida que la ronda 5 pidió expresamente ("una ronda dirigida EXCLUSIVAMENTE a poblar esa franja baja con contenido real... si tras esa ronda dirigida el total se estanca de nuevo por debajo del gate pese a resolver ese punto, ahí sí correspondería evaluar el cierre por criterio propio") se ejecutó, se verificó, y resolvió exactamente lo que debía resolver: el defecto estructural único y dominante que 4 rondas consecutivas (2, 3, 4, 5) señalaron con nombre, ubicación y diagnóstico idéntico ya no existe — craft cruza su gate (16/20, con profundidad en 4/4) y h10 sube a 4/4. Sin embargo, la usabilidad total se estanca en 32/40, 4 puntos bajo el gate, tal como anticipó la ronda 5 como escenario posible. La diferencia clave con las rondas 2-5 es que ahora YA NO hay un defecto único y dominante que explique la brecha: lo que queda es la suma dispersa de 7 heurísticas ancladas en "bien" (3/4) por razones de ALCANCE (un solo atajo en h7, 5 opciones de decisión en vez de 4, un matiz de copy, un radio de 2-4px fuera de familia) más un defecto puntual fácil de arreglar (el botón nuevo sin padding) que por sí solo no cierra la brecha de 4 puntos. Subir cada una de esas 7 heurísticas de "bien" a "ejemplar" requeriría intervenciones de producto independientes entre sí (agregar más atajos, reducir la decisión a 4 opciones, refinar copy al límite) — exactamente el patrón de "matices dispersos de bajo impacto" que en Coach y Diario llevó a cerrar con criterio propio pese a no cruzar el gate numérico completo. Recomendación: corregir el defecto puntual #1 de esta ronda (botón sin padding/feedback, arreglo de una línea) por higiene y consistencia, y cerrar la pantalla con criterio propio — seguir puntuando rondas adicionales sobre esta misma captura entra en rendimientos decrecientes reales, ya sin un blanco estructural identificable como el que hubo hasta la ronda 5.
